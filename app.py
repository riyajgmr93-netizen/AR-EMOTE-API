...
try:
    with open("emotes.json", "r", encoding="utf-8") as f:
        emotes_list = json.load(f)
        VALID_EMOTE_IDS = {int(item["Id"]) for item in emotes_list}
    print(f"Loaded {len(VALID_EMOTE_IDS)} emote IDs from emotes.json")
...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    major_login.client_version = "2.131.1"
...
def join_team():
    ...
    # Yahan paste karein (Line 333 ke paas):
    if VALID_EMOTE_IDS and emote_id not in VALID_EMOTE_IDS:
        return jsonify({
            "status": "error",
            "message": f"Invalid emote_id: {emote_id}. emotes.json me nahi mila!"
        }), 400
...
async def MaiiiinE():
    ...
    # BOT LOGIN UID
    BOT_UID = int('18034155137')  # <-- FIXED BOT UID

    Uid, Pw = '7685638882', 'kaddu_d5c'
